...
class Player:
    def __init__(self, _frame, _x, _y, _type, _score=0):
        ...
        self.superCanvasItem = self.canvas.create_polygon(playerCanvasSize / 2, 0 + 4, \
                                                          playerCanvasSize - 4, playerCanvasSize / 2, \
                                                          playerCanvasSize / 2, playerCanvasSize - 4, \
                                                          0 + 4, playerCanvasSize / 2, \
                                                          fill=self.col1, outline=colour["gold"], width=4)
        self.update()
    ...
    def update(self):
        if not self.visible: self.canvas.place_forget()
        ...
        if farEnough and self.visible: self.canvas.place(x=self.xTo, y=self.yTo)
        ...
        # Dans le cas ou on est devenu super
        if self.super:
            self.col2 = colour["gold"]
            self.canvas.itemconfig(self.superCanvasItem, state=NORMAL)
        # Sinon,
        else:
            self.col2 = colour["whitePlayer"] if self.type == +1 else colour["blackPlayer"]
            self.canvas.itemconfig(self.superCanvasItem, state=HIDDEN)
    ...
    def __init__(self, _frame, _gSize, _bSize):
        for j in range(_gSize):
            for i in range(_gSize):
                # Création d'une liste temp pour les joueurs mangés et ceux super
                eatenPlayers[i][j] = []
                superPlayers[i][j] = False
...
def updateGame():
    global aiState, aiCoords, aiInCombo, gameEnd, players
    # Si la variable restart == 1, on reset le game
    if restart.get() == 1: resetGame()
    # Si la variable undo == 1, on annule le dernier mouvement
    if undo.get() == 1: undoMovement()
    if not gameEnd:
        ...
        if aiState == None:
            ...
            aiMove(aiCoords[0], aiCoords[1], combo=aiInCombo)
    # Se ré-appeler pour mettre à jour
    window.after(refreshRate, updateGame)

"""Fonction qui annule le dernier mouvement"""
def undoMovement():
    global player, players, scorePlayer, eatenPlayers, superPlayers, scoreDisplay, \
        aiState, initialCount, highlightStuck, selectedPlayer
    # On retrouve le dernier état de jeu
    lastIndex = len(optionvars.gSaves) - 1
    save = optionvars.gSaves[lastIndex]
    # On parcourt la grille
    for j in range(gSize):
        for i in range(gSize):
            # Si il y a un jeton actuel,
            if players[i][j] != -1:
                # Retrouver les anciennes coordonnées de ce jeton
                oldPlayer = listCompare(players[i][j], save[0])
                # Changer la position du jeton actuel à celle de l'ancien jeton
                if oldPlayer != -1:
                    players[i][j].changePosition(oldPlayer[0], oldPlayer[1])
            # Retrouver les coordonnées récentes des anciens jetons
            newPlayer = listCompare(save[0][i][j], players)
            # Si la nouvelle version du jeton n'a pas été retrouvée,
            if newPlayer == -1:
                mostRecentEatenPlayerIndex = len(eatenPlayers[i][j]) - 1
                # Alors faire revenir un ancien joueur mangé
                players[i][j] = copy.deepcopy(eatenPlayers[i][j][mostRecentEatenPlayerIndex])
                # Reset la liste des joueurs mangés
                del eatenPlayers[i][j][mostRecentEatenPlayerIndex]
                # Replacer le canevas du joueur mangé
                players[i][j].visible = 1
                players[i][j].canvas.place(x=players[i][j].x, y=players[i][j].y)
    # Appliquer la liste en soi
    players = copy.deepcopy(save[0])
    # On reparcourt la grille pour reset les formes super
    for j in range(gSize):
        for i in range(gSize):
            if players[i][j] != -1:
                superPlayers = copy.deepcopy(save[1])
                players[i][j].super = superPlayers[i][j]
    # Revenir un tour en arrière
    player = save[2]
    if optionvars.ai == 1 and player == -optionvars.humanPlayer:
        initialCount = counter.value2
        aiState = None
    if (optionvars.ai == 0) or (optionvars.ai == 1 and player == optionvars.humanPlayer):
        aiState = True
    # On parcourt les joueurs du score
    for k in [-1, 1]:
        for l in range(gSize * 2):
            # Retrouver les anciennes coordonnées des jetons score
            oldScorePlayer = dictCompare(scorePlayer[k][l], save[3])
            # Si l'ancienne version du jeton score n'a pas été retrouvée,
            if oldScorePlayer == -1:
                # Détruire ce jeton score
                scorePlayer[k][l].canvas.destroy()
                scorePlayer[k][l] = -1
    # Appliquer la liste en soi
    scorePlayer = copy.deepcopy(save[3])
    # Revenir à l'ancien score
    for m in [-1, 1]:
        scoreDisplay[m].set(save[4][m])
    # Revenir aux anciennes variables
    highlightStuck = save[5]
    selectedPlayer = -1
    if highlightStuck:
        selectedPlayer = save[6]
        highlight(selectedPlayer.i, selectedPlayer.j, player)
    # On supprime la sauvegarde de l'état de jeu
    del optionvars.gSaves[lastIndex]
    # Ne plus exécuter
    undo.set(0)
...
def resetGame():
    global b, players, scoreDisplay, onePlayerCanEat, selectedPlayer, \
        ...
        aiCoords, aiState, aiInCombo, moves, initialCount, gameEnd, eatenPlayers, superPlayers
    # On loop dans la grille
    for j in range(gSize):
        for i in range(gSize):
            # Remettre à 0 la liste des joueurs mangés et ceux super
            eatenPlayers[i][j] = []
            superPlayers[i][j] = False
    ...
    scorePlayer = {-1: [-1 for i in range(gSize * 2 + 1)], 1: [-1 for i in range(gSize * 2 + 1)]}
    counter.value = 0
    counter.value2 = 0
    initialCount = 0
    aiState = None
...
def canMove():
    ...
    for be in [-1, -2]:
        ...
        for pi in range(gSize):
            for pj in range(gSize):
                # Mettre à jour la liste des super jetons (non lié au mouvement)
                if players[pi][pj] != -1: superPlayers[pi][pj] = players[pi][pj].super
                # S'il y a un jeton et s'il est du joueur actuellement en train de jouer
                if players[pi][pj] != -1 and players[pi][pj].type == player:
...
def highlight(i, j, player, behaviour=1):
    ...
    for playerVar in direction1:
        for direction in direction2:
            ...
            outsideGrid = (not isBetween(ni(1), 0, gSize - 1) or not isBetween(nj(1), 0, gSize - 1))
            # Un jeton de même type (1 case devant)
            samePlayerCondition = not outsideGrid and players[ni(1)][nj(1)] != -1 and players[ni(1)][nj(1)].type == player
            # Un jeton de type opposé (1 case devant)
            otherPlayerCondition = not outsideGrid and players[ni(1)][nj(1)] != -1 and players[ni(1)][nj(1)].type == -player
...
def eat(i, j, player, playerMovement):
    ...
    eatenPlayer.visible = 0
    eatenPlayers[eatenPlayerX][eatenPlayerY].append(copy.deepcopy(players[eatenPlayerX][eatenPlayerY]))
...
def click(event, i, j):
    global player, selectedPlayer, highlightStuck, onePlayerCanEat, nothingHappened, onePlayerCanMove, \
        initialCount, aiCoords, aiState, aiInCombo, gameEnd, superPlayers
    ...
    # Si on est sur une case sans jeton sélectionnable,
        ...
        if c[i][j] == colour["green"] and players[i][j] == -1:
            # On enlève les surbrillances
            resetCaseColour()
            ...
            tempScorePlayer = copy.deepcopy(scorePlayer)
            tempScoreDisplay = copy.deepcopy(scoreDisplay)
            tempSuperPlayers = copy.deepcopy(superPlayers)
            optionvars.gSaves.append([tempPlayers, tempSuperPlayers, player, tempScorePlayer, tempScoreDisplay, \
                                      highlightStuck, selectedPlayer])
            ...
            # Si on est dans un combo en tant que IA
            elif event == None:
                # Ré-appeler la fonction choisissant nos mouvements pour continuer la séquence de mouvements
                aiCoords = aiMoveChoice(specificPlayer=(i, j))
                aiInCombo = 1
    ...
    # On change l'état du bouton "annuler"
    if len(optionvars.gSaves) <= 0 or optionvars.difficulty > 1:
        gameCancelText.disabled = 1
    else:
        gameCancelText.disabled = 0
    # On check si quelqu'un a gagné
    victory()

"""Fonction qui détermine le gain d'un mouvement"""
def movementGain(token, move, _player, _players, _superPlayers):
    tokenGain = 0
    # Si on bouge pour devenir un super jeton
    try: superVar = _players[token[0]][token[1]].super
    except: superVar = _superPlayers[token[0]][token[1]]
    if ((move[1] == 0 and _player == -1) or (move[1] == gSize - 1 and _player == 1)) and not superVar:
        tokenGain += 2
    # Si on mange un jeton
    movement = movementVector(token[0], token[1], move[0], move[1])
    if movement[2] > sqrt(2):
        tokenGain += 1
    # Si on bouge ou reste devant un jeton qui peut nous manger
    for p in [token, move]:
        for direction in [-1, 1]:
            if isBetween(p[0] + direction, 0, gSize-1) and isBetween(p[1] + _player, 0, gSize-1) \
            and isBetween(p[0] - direction, 0, gSize-1) and isBetween(p[1] - _player, 0, gSize-1):
                if _players[p[0] + direction][p[1] + _player] != -1:
                    #try:
                    typeVar = _players[p[0] + direction][p[1] + _player].type
                    #except: typeVar = int(_players[p[0] + direction][p[1] + _player])
                    behindIsEmpty = _players[p[0] - direction][p[0] - _player] == -1
                    if (_players[p[0] + direction][p[1] + _player] != -1 and typeVar == -_player and behindIsEmpty):
                        tokenGain -= 1.5
    # Si on se déplace vers une bordure
    if move[0] == 0 or move[0] == gSize-1:
        tokenGain += 1.25
    return tokenGain
            ...
            for pj in range(gSize):
                if _players[pi][pj] != -1 and int(_players[pi][pj]) == _player:
                    if aiHighlight(pi, pj, _player, _players, _superPlayers, behaviour=be):
                        # S'il peut manger
                        if be == -1:
                            _onePlayerCanEat[_player].append((pi, pj))
                            _onePlayerCanMove[_player].append((pi, pj))
                        # S'il peut bouger
                        if be == -2:
                            _onePlayerCanMove[_player].append((pi, pj))
        if be == -1:
            # Si le joueur peut effectivement en manger un autre,
            if _onePlayerCanEat != {-1: [(-1, -1)], 1: [(-1, -1)]}:
...
def aiHighlight(i, j, _player, _players, _superPlayers, behaviour=-3):
    ...
    _possibleMoves = []
    direction1 = [_player]
    if _players[i][j] != -1 and _superPlayers[i][j]: direction1.append(-_player)
    direction2 = [-1, 1]
    for playerVar in direction1:
        for direction in direction2:
            ni = lambda a: i - a * direction
            nj = lambda a: j + a * playerVar
            outsideGrid = (not isBetween(ni(1), 0, gSize - 1) or not isBetween(nj(1), 0, gSize - 1))
            samePlayerCondition = not outsideGrid and _players[ni(1)][nj(1)] != -1 and int(_players[ni(1)][nj(1)]) == _player
            otherPlayerCondition = not outsideGrid and _players[ni(1)][nj(1)] != -1 and int(_players[ni(1)][nj(1)]) == -_player
            outsideGrid = (not isBetween(ni(1), 0, gSize - 1) or not isBetween(nj(1), 0, gSize - 1)) \
                           or (otherPlayerCondition and (not isBetween(ni(2), 0, gSize - 1)
                                                         or not isBetween(nj(2), 0, gSize - 1)))
            if outsideGrid: continue
            if otherPlayerCondition:
                if _players[ni(2)][nj(2)] == -1:
                    if behaviour == -1: return True
                    if behaviour == -3: _possibleMoves.append((ni(2), nj(2)))
                else:
                    continue
            if samePlayerCondition: continue
            if _players[ni(1)][nj(1)] == -1:
                if onePlayerCanEat[_player] == [(-1, -1)] and not highlightStuck:
                    if behaviour == -2: return True
                    _possibleMoves.append((ni(1), nj(1)))
    if behaviour == -3: return _possibleMoves
    else: return False

"""Fonction itératrice des mouvements"""
def minMax(allMoves, tempPlayers, tempSuperPlayers, depth, originalMove=((0, 0), (0, 0)), specificPlayer=None):
    global players
    ...
    for t, token in enumerate(playersToCheck):
        ...
        for m, move in enumerate(possibleMoves):
            if depth == 0: originalMove = (token, move)
            # On leur assigne un gain et on les ajoute à l'arbre de mouvements
            tempGain = movementGain(token, move, aiPlayer, tempPlayers, tempSuperPlayers)
            if tempGain > allMoves[0]:
                allMoves[0] = tempGain
                allMoves.append(originalMove)
            ...
            movement = movementVector(token[0], token[1], move[0], move[1])
            if movement[2] > sqrt(2):
                tempPlayers[token[0] + int(movement[0]/2)][token[1] + int(movement[1]/2)] = -1
            # Simulation d'un mouvement humain
            # (NE MARCHE PLUS CAR CETTE FONCTION APPELLE CANMOVE()/HIGHLIGHT() AU LIEU DE AICANMOVE()/AICANHIGHLIGHT())
            humanMove = aiSimpleChoice(-aiPlayer, tempPlayers, tempSuperPlayers, specificPlayer=specificPlayer)
            # On modifie les listes virtuelles des joueurs et des super joueurs
            ...
            movement = movementVector(humanMove[0][0], humanMove[0][1], humanMove[1][0], humanMove[1][1])
            if movement[2] > sqrt(2):
                tempPlayers[humanMove[0][0] + int(movement[0]/2)][humanMove[0][1] + int(movement[1]/2)] = -1
            # Repeat from the start (get all move possibilities, etc)
            if depth < optionvars.difficulty * 2:
                depth += 1
                allMoves = minMax(allMoves, tempPlayers, tempSuperPlayers, depth, \
                                  originalMove=originalMove, specificPlayer=specificPlayer)
            else: depth = 0
    return allMoves
...
def aiRandomChoice(specificPlayer=None):
    ...
    if specificPlayer == None:
        try:
            playerToMove = random.choice(onePlayerCanMove[-optionvars.humanPlayer][1:])
        except:
            return
    else:
...
def aiSimpleChoice(_player, _players, _superPlayers, specificPlayer=None):
    # Initialisation
    maxGain = -999
    bestMoves = list()
    if specificPlayer == None:
    ...
    for token in playersToCheck:
        possibleMoves = highlight(token[0], token[1], _player, behaviour=-3)
        for move in possibleMoves:
            tempGain = movementGain(token, move, _player, _players, _superPlayers)
            if tempGain > maxGain:
                bestMoves = list()
...
def aiMoveChoice(specificPlayer=None):
    ...
    # En difficulté 1
    elif optionvars.difficulty in [1, 2]:
        bestMove = aiSimpleChoice(aiPlayer, players, superPlayers, specificPlayer=specificPlayer)
...
def aiMove(playerToMove, targetMove, combo=0):
    ...
    if combo == 1:
        # Au moment venu,
        if (deltaTime == (interval * round(moveTime + comboTime, 1))) \
        or (deltaTime > (interval * round(moveTime + comboTime, 1)) \
        and (deltaTime - interval * round(moveTime + comboTime, 1)) % (interval * comboTime) == 0):
...
# IA
aiState = None
if (optionvars.humanPlayer == player or optionvars.ai == 0 or currentFrame != gameFrame): aiState = True
