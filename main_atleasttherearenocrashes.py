...
def highlight(i, j, player, behaviour=1):
    ...
    for playerVar in direction1:
        for direction in direction2:
            ...
            samePlayerCondition = players[ni(1)][nj(1)] != -1 and players[ni(1)][nj(1)].type == player
            # Un jeton de type opposé (1 case devant)
            otherPlayerCondition = players[ni(1)][nj(1)] != -1 and players[ni(1)][nj(1)].type == -player
...
def movementGain(token, move, _player, _players, _superPlayers):
    ...
    for direction in [-1, 1]:
        try: typeVar = _players[move[0] + _player * direction][move[1] + _player].type
        except: typeVar = int(_players[move[0] + _player * direction][move[1] + _player])
        if (_players[move[0] + _player * direction][move[1] + _player] != -1 and typeVar == -_player):
            tokenGain -= 1
...
def aiHighlight(i, j, _player, _players, _superPlayers, behaviour=-3):
    ...
    for playerVar in direction1:
        for direction in direction2:
            ni = lambda a: i - a * direction
            nj = lambda a: j + a * playerVar
            samePlayerCondition = _players[ni(1)][nj(1)] != -1 and int(_players[i][j]) == _player
            otherPlayerCondition = _players[ni(1)][nj(1)] != -1 and int(_players[i][j]) == -_player
            outsideGrid = (not isBetween(ni(1), 0, gSize - 1) \
                           or not isBetween(nj(1), 0, gSize - 1)) \
                          or (otherPlayerCondition and ( \
                        not isBetween(ni(2), 0, gSize - 1) \
...
def aiSimpleChoice(_player, _players, _superPlayers, specificPlayer=None):
    ...
    for token in playersToCheck:
        possibleMoves = aiHighlight(token[0], token[1], _player, _players, _superPlayers, behaviour=-3)
        for move in possibleMoves:
