...
def highlight(i, j, player, behaviour=1, specificPlayers = None):
...
def click(event, i, j):
    ...
    if len(optionvars.gSaves) <= 0: gameCancelText.disabled = 1
...
def aiMoveChoice(specificPlayer=None):
    ...
    # En difficulté 1
        ...
        bestmove = aiGain(-optionvars.humanPlayer, players, 0, specificPlayer=specificPlayer)
...
def aiGain(tempPlayer, players, behaviour=None, specificPlayer=None):
    # behaviour = 0, appel normal de la fonction
    # behaviour = 1, appel recursive pour l'enchaînement des combos
    ...
    for token in playersToCheck:
        if behaviour == 1:
            possibleMoves = highlight(token[0], token[1], tempPlayer, behaviour=-3, specificPlayers=players)
        else:
        ...
        for move in possibleMoves:
            ...
            if move[0] == 0 or move[0] == 9:
                tempgain += 1.5
            # Si on mange(par jeton mangé)
            if move[1]-token[1] < -1 or move[1]-token[1] > 1:
                tempgain += 2
                tempplayers = copy.deepcopy(players)
                temptoken = move
                tempplayers[move[0]][move[1]] = tempplayers[token[0]][token[1]]
                tempplayers[token[0]][token[1]] = -1
                tempplayers[int((token[0]+move[0])/2)][int((token[1]+move[1]/2))] = -1
                while True:
                    possibleMoves = highlight(temptoken[0], temptoken[1], tempPlayer, behaviour=-3, specificPlayers=tempplayers)
                    if len(possibleMoves) == 0:
                        break
                    for futuremove in possibleMoves:
                        if futuremove[1]-temptoken[1] < -1 or futuremove[1]-temptoken[1] > 1:
                            tempgain +=2
                            tempplayers[futuremove[0]][futuremove[1]] = tempplayers[temptoken[0]][temptoken[1]]
                            tempplayers[temptoken[0]][temptoken[1]] = -1
                            tempplayers[int((temptoken[0] + futuremove[0]) / 2)][int((temptoken[1] + futuremove[1] / 2))] = -1
                            temptoken = futuremove
                            continue
                    break
